...
class DataTrainingArguments:
    ...
    test_file: Optional[str] = field(
        default=None, metadata={"help": "The data file (a text file) for predict."})
...
def main():
    ...
    tokenizer_kwargs = {
        ...
        "revision": model_args.model_revision,
        "use_auth_token": True if model_args.use_auth_token else None,
        "padding_side": 'left' if training_args.do_predict else 'right'
    }
    ...
    if training_args.do_predict:
        ...
        logger.info("**"*15 + "Predict" +  "**"*15)
        print(model.generation_config)

        # num_samples = len(tokenized_dataset['test'])
        # for i in tqdm(range(0, num_samples, 2),  total=math.ceil(num_samples)):
        
        results = []
        for i, line in tqdm(enumerate(tokenized_dataset['test']), total=len(tokenized_dataset['test'])):
            for k in line.keys(): # list->tensor
                line[k] = torch.LongTensor([line[k]]).to(training_args.device)
            # output_ids = model.generate(**tokenized_dataset['test'][i:i+2],
            output_ids = model.generate(**line,
                                        generation_config=model.generation_config,
                                        max_new_tokens=512) # 返回的是二维[1,seqlen]，故下面需要取[0]
            output_str = tokenizer.batch_decode(output_ids)[0] 
            try:
                # target = [x.split('答：\n')[1].split('</s>') for x in output_str]
                target = output_str.split('答：\n')[1].split('</s>')[0]
            except:
                print("Error on sample_id=%s:\n" % raw_datasets['test'][i]['sample_id'], output_str)
                target = ""
                
            res = copy(raw_datasets['test'][i])
            res['target'] = target
            results.append(res)

        print(results[:2])
        with jl.open(os.path.join(training_args.output_dir, 'test_predictions.jsonl') , 'w') as writer:
